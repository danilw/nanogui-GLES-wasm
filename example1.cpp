...
public:
    ExampleApplication() : nanogui::Screen(Eigen::Vector2i(1024, 768), "NanoGUI Test", /*resizable*/true, /*fullscreen*/false, /*colorBits*/8,
        ...
        auto imageView = new ImageView(imageWindow, mImagesData[0].first.texture(), 256, 256);
        mCurrentImage = 0;
        // Change the active textures.
        imgPanel->setCallback([this, imageView](int i) {
            imageView->bindImage(mImagesData[i].first.texture(), 256, 256);
            mCurrentImage = i;
        ...
        b->setCallback([&] {
			cout << "File dialog not implemented"<< endl;
        });
        ...
        Widget *fpspanel = new Widget(window);
        fpspanel->setLayout(new BoxLayout(Orientation::Horizontal,
                                       Alignment::Middle, 0, 6));
        new Label(fpspanel, "FPS:", "sans-bold");
        fpscapt = new TextBox(fpspanel);
        fpscapt->setFixedSize(Vector2i(60, 25));
        fpscapt->setValue("0");
        fpscapt->setUnits(" ");
        fpscapt->setFixedSize(Vector2i(60,25));
        fpscapt->setFontSize(25);
        fpscapt->setAlignment(TextBox::Alignment::Right);
