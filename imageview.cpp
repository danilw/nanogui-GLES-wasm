...
ImageView::ImageView(Widget* parent, GLuint imageID, int w, int h)
...
void ImageView::bindImage(GLuint imageId, int w, int h) {
	setImageSize(w,h);
    mImageID = imageId;
    updateImageParameters();
    fit();
...
void ImageView::zoom(int amount, const Vector2f& focusPosition) {
    ...
    mScale = std::min(100.f,std::max(0.1f, scaleFactor * mScale)); // GLES/wasm fix
...
void ImageView::updateImageParameters() {
    ...
    //mImageSize = Vector2i(w, h);
    //std::cout << "Warning: ImageVIew::updateImageParameters(): not supported on GLES!" << std::endl;;
}
